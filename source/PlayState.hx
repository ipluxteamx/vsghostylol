...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'despair':
				dialogue = CoolUtil.coolTextFile(Paths.txt('despair/dialogue'));
			case 'temper':
				dialogue = CoolUtil.coolTextFile(Paths.txt('temper/dialogue'));
			case 'anger':
				...
				{
					{
						...
						bg = new FlxSprite(-600, -200).loadGraphic(Paths.image('modStages/amoguschair_bg'));
						bg.antialiasing = true;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (!isStoryMode == true || curSong == 'bopeebo' || curSong == 'tutorial' || curSong == 'spookeez' || curSong == 'pico'
				|| curSong == 'satin-panties' || curSong == 'cocoa' || curSong == 'senpai' || curSong == 'despair') // Just play the intro anim for JUST the first song in a week. I think theres a better way to do it tho.
			{
				babyArrow.y -= 20;
				babyArrow.alpha = 0;
...
/*	function noteMiss(direction:Int = 1):Void
	...
	**/
	...
	override function beatHit()
	{
		...
		//totally didn't steal that from hellchart carol what you talkin about
		if (dad.curCharacter == "ghosty" || dad.curCharacter == "madghosty") {
		...
		if (curBeat % 16 == 14 && curSong == 'Insanity')
			{
				// idk im lazy asf
				strumLineNotes.forEach(function(note)
				{
